...
static uint balloc(uint dev, uint n)
{
  ...
  for (b = 0; b < sb.size; b += BPB) {
    bp = bread(ROOTDEV, BBLOCK(b, sb)); // look through each bitmap sector
...
// appending to the end of the inode table. The inodetable has an
...
// Find the inode table on the disk and load it into memory
