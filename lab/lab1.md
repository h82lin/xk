...
and most importantly, implement a number of system calls for xk. There are questions throughout the spec that aims to enhance your understanding of xk.
**As a group**, you need to answer these questions on the corresponding Gradescope assignment.
...
You can develop on `attu` using a text editor of your choice. If you want to use VSCode, checkout [`vscode.md`](../docs/development/vscode.md).

Once you are logged in to `attu`, you can clone the course repository by running the commands below.

```
$ git clone git@gitlab.cs.washington.edu:xk-public/24sp.git xk
Cloning into xk...
$ cd xk
```
...
4. Add all course staffs as Developer to your repo, make sure to add via emails (shown [here](https://courses.cs.washington.edu/courses/cse451/24sp/index.html#staff))
...
```
export PATH=/cse/courses/cse451/24sp/bin/x86_64-softmmu:$PATH
```
You can then run 
```
source ~/.bashrc
...
[this section](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/kernel/entry.S#L126-139) after the switch to 64-bit.
...
xk will go through the booting and stop at `main`. 

Which line of code in `main` (a) prints the cpu information and (b) prints the physical memory table? 
Please write down both the line number and the code in the line. (Hint: use the `n` command)
...
Does GDB work with virtual or physical addresses? In other words,
when GDB prints out the address of variables and functions, are they virtual or physical addresses?
Please justify your answer.

---

### Memory Management in xk
Next, we will take a look at xk's virtual memory design. Please read [memory management in xk](../docs/xk/memory.md) and answer the questions below.

##### Question #3
Why does xk map kernel and user-application into the same address space? 
In other words, why doesn't the kernel have its own address space?

Please provide 2 reasons and explain them.

##### Question #4

> Note: for both of the following questions, please provide at least one difference that is meaningful at the API level.
> We are not asking for differences in its function signatures, but the design differences between them.
> Is each pair interchangable? Do the functions operate on the same abstraction level? If not, what does the difference buy them?
...
Each supported system call in [inc/user.h](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/inc/user.h#L8-L30)
...
In xk, when a process invokes a system call listed in [user/user.h](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/inc/user.h)
it goes to the provided system call stubs in [user/lib/usys.S](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/user/lib/usys.S). 
...
through a software interrupt (line 8) with the interrupt number for system calls (`TRAP_SYSCALL` in [inc/trap.h](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/inc/trap.h#L30)).

Once in kernel mode, the software interrupt is captured by the registered trap vector ([kernel/vectors.S](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/kernel/vectors.S#L316-L319)).
The trap vector saves the interrupt number (`TRAP_SYSCALL`) and the process's registers onto its kernel stack (`alltraps`), 
creating the [trapframe](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/inc/trap.h#L41-L66).
...
##### Question #5
What is the first line of c code executed in the kernel when a software or hardware interrupt occurs?
You can cause a software interrupt by performing a system call. Add a `sleep` call within `lab1test.c` and use gdb to trace through it with the `si` command.
Our gdb script loads the kernel symbols on startup via the `kern` command. You would also want to load in the init program's symbols by running the `initcode` command in gdb.
...
##### Question #7
Set a breakpoint at `sys_sleep` and continue executing until the breakpoint is hit (be sure to call `sleep` within `lab1test.c`). 
Do a backtrace (`bt`) in gdb. What is the kernel code path (reported by `bt`) that led to `sys_sleep`? 
...
The system call interface allows users to pass various types of arguments (`char*`, `void *`, `int`, etc.).
When the kernel receives syscall arguments, is it safe to use them without validation? 
How might a user process exploit the xk kernel (e.g. read/write kernel data) if there is no validation to the system call? 
...
Each process is allowed to have a maximum of `NOFILE` ([inc/param.h](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/inc/param.h)) amount of open files.
...
Once you define your struct, you need to manage its allocation and deallocation. The kernel has a hard limit on the total number of open files 
across all processes (checkout `inc/param.h`), this means that you can allocate a static array of `struct file_info` as your global file info table.
...
> image `out/fs.img` written by [mkfs.c](https://gitlab.cs.washington.edu/xk-public/24sp/-/blob/main/mkfs.c).
